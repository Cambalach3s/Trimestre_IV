...
router.get('/:tdoc_vendedor/:id', (req, res) => {
    const { tdoc_vendedor, id } = req.params;
    const query = `SELECT * FROM vendedor WHERE tdoc_vendedor='${tdoc_vendedor}' AND id_vendedor='${id}'`;
    pool.query(query, (error, resultado) => {
        if (error) {
            console.error(error.message);
            return res.status(500).json({ error: 'Error en la consulta SQL' });
...
router.delete('/borrar/:tdoc_vendedor/:id', (req, res) => {
    const { tdoc_vendedor, id } = req.params;

    const query = `DELETE FROM vendedor WHERE tdoc_vendedor='${tdoc_vendedor}' AND id_vendedor='${id}'`;
